...
public class OmsOrderServiceImpl extends ServiceImpl<OmsOrderMapper, OmsOrder> implements OmsOrderService {
    ...
    public OmsOrder generateOrder(OrderParamDTO orderParamDTO) {
        ...
        queryWrapper.lambda().eq(OmsCartItem::getMemberId,currentMember.getId())
                .in(OmsCartItem::getId,orderParamDTO.getItemIds());
        //根据购物车id查询所有信息
        List<CartItemStockDTO> cartItemStorkByIds=cartItemMapper.getCartItemStorkById(queryWrapper);

        //1.判断库存（如果没有库存直接提示）
        //获取库存不足的名称 如果productName为空说明库存未超出
        String productName=hasStock(cartItemStorkByIds);
        if(StrUtil.isNotEmpty(productName)){
            throw new ApiException("您的手速过慢，"+productName+"已被别人买走");
        }
        //如果有库存就进行下单操作：
        //2.保存订单主表oms_order信息  订单号
        OmsOrder omsOrder=newOrder(orderParamDTO,currentMember,cartItemStorkByIds);
        this.save(omsOrder);
        //3.保存订单相情表 oms_order_item(购物车转移)
        List<OmsOrderItem> List=newOrderItems(omsOrder,cartItemStorkByIds);
        orderItemService.saveBatch(List);
        //4.锁定库存
        lockStock(cartItemStorkByIds);
        //5.删除对应购物车
        removeCartItem(cartItemStorkByIds);
    ...
    public void cancelOverTimeOrder() {
        ...
        orderQueryWrapper.lambda().eq(OmsOrder::getStatus,0)
                .lt(OmsOrder::getCreateTime,offset); //是否超时
        List<OmsOrder> list = this.list(orderQueryWrapper);
        ...
        itemQueryWrapper.lambda().eq(OmsOrderItem::getOrderId,orderIds);
    ...
    // 创建OmsOrder
    private OmsOrder newOrder(OrderParamDTO orderParamDTO, UmsMember currentMember, List<CartItemStockDTO> cartItemStorkByIds) {
        OmsOrder omsOrder = new OmsOrder();
        ...
        for (CartItemStockDTO cartItem : cartItemStorkByIds) {
        ...
        addressQueryWrapper.lambda().eq(UmsMemberReceiveAddress::getMemberId,currentMember.getId())
                .eq(UmsMemberReceiveAddress::getId,orderParamDTO.getMemberReceiveAddressId());
    ...
    /**
     * 判断是否有库存
     * @param cartItemStorkByIds
     * @return
     */
    private String hasStock(List<CartItemStockDTO> cartItemStorkByIds) {
        for (CartItemStockDTO cart:cartItemStorkByIds){
